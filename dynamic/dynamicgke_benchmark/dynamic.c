...
int main(){
	...
	//uint32_t b_alice[1024]; // n=1024
	//uint32_t b_bob[1024];
	//uint32_t b_david[1024];	

	//rlwe_kex_generate_keypair(a, s_alice, b_alice, &ctx);
	//rlwe_kex_generate_keypair(a, s_bob, b_bob, &ctx);
	//rlwe_kex_generate_keypair(a, s_david, b_david, &ctx);
	...
	for(int t=0; t<1024; t++){
		tmp[t]=pub_keys[2][t];
		tmp2[t]=augmented_pub_keys[2][t]; 
	}	
	
	FFT_mul(tmp, tmp, s_bob, &ctx); // tmp=z_i-1*s_i 
	FFT_add(tmp, tmp2, tmp); // tmp=X_i+tmp

	for(int t=0; t<1024; t++){
		Y[2][t]=tmp[t]; // Y[i] 저장 (tmp)
		tmp2[t]=augmented_pub_keys[3][t]; // tmp2=X[peer+1]
	}
	
	for (int j=1; j<num_peer; j++){
		FFT_add(tmp, tmp, tmp2); // Y[i]=Y[i-1]+X[i]
		for(int t=0; t<1024; t++){
			Y[(2+j)%num_peer][t]=tmp[t]; // Y[2+j] 저장 (tmp)
			tmp2[t]=augmented_pub_keys[(2+j+1)%num_peer][t]; // tmp2=X[2+j+1]
		}
	}
	...
	rlwe_rec(k_charlie, result, rec);
	sha512_session_key(k_charlie, hk_charlie);
	...
	for (int i = 0; i < 16; i++) {
		...
		keys_match &= (k_eve[i] == k_charlie[i]);
	}
	...
	for (int i = 0; i < 129; i++) {
		...
		hkeys_match &= (hk_eve[i] == hk_charlie[i]);
		hkeys_match &= (hk_alice[i] == hk_charlie[i]);
	}
